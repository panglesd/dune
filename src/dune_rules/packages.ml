...
let assets_by_package = Memo.With_implicit_output.exec assets_by_package_def

(* TODO memoize this so that we can cutoff at the package *)
let assets sctx pkg =
  let+ map = assets_by_package sctx in
  Package.Name.Map.find map pkg |> Option.value ~default:[]
;;
